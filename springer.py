...
def run_springer():
    with open(logfile, 'a+', encoding='utf-8') as f:
        f.write('springer_spider正常启动:%s' % (time.strftime('%Y.%m.%d %H:%M:%S')) + '\n')
    try:
        ...
        with open(logfile, 'a+', encoding='utf-8') as f:
            ...
            f.write('springer_spider异常停止:%s' % (time.strftime('%Y.%m.%d %H:%M:%S')) + str(e) + '\n\n')
    else:
        with open(logfile, 'a+', encoding='utf-8') as f:
            f.write('springer_spider正常停止:%s' % (time.strftime('%Y.%m.%d %H:%M:%S')) + '\n\n')
